...
public class Okno6 {
    ...
    private void przygotujZdarzenia() {
        //utowrzenie nowego listenera i powołanie na zdarzenia z zdefiniowanego listenera
        ...
        guzik.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                ...
                powitanie.setText("Witam " + imie);


            }
        });

     //wyrazenie lambda - nowa składnie
        guzik2.addActionListener(ev -> {
            int kolor=ThreadLocalRandom.current().nextInt(0x1000000); // losuje kolor
            guzik2.setForeground(new Color(kolor)); // nadanie napisowi na przycisku nowego koloru - wylosowanego
            int dx = ThreadLocalRandom.current().nextInt(200); // losuję zmienną x
            int dy = ThreadLocalRandom.current().nextInt(200); // losuję zmienną y
            okno.setLocation(okno.getX() + dx-100, okno.getY() +dy -100); // podstawiam dx pod x i dx pod y i dodatkowo zmieniamy o 100
        });
